...
def run_simulation_merge():
    ...
    for id in vehicles_lanes.keys():
        ...
        if vehicles_lanes[id][3][0].text == '10':  # ingress
            ...
            traj_id = id + '-' + gekoppelde_egresslane  # define the id
            traj_coordinates = get_coordinates(root, lane_ing, 'trajectory')  # get coordinates of trajectory
            traj_length = calculate_trajectory(traj_coordinates[0][1], traj_coordinates[0][0], traj_coordinates[-1][1],
                                               traj_coordinates[-1][0])  # calculate the length
            lane_objects[traj_id] = Lane(traj_id, traj_coordinates,
                                         'trajectory')  # define Lane object and add it ti the dict

        else:  # egress
            lane_objects[id] = Lane(id, coordinaten, 'egress', signal)

    # # define a list of Cars objects
    car1 = Car(Location(lane_objects.get('1'), lane_objects.get('1').length - 1), 1, 2,
               False)  # self, id: str, length: int, speed:int, start_posistion, destination
    car2 = Car(Location(lane_objects.get('1-26'), lane_objects.get('1-26').length - 1), 1, 2, False)
    car3 = Car(Location(lane_objects.get('26'), lane_objects.get('26').length - 1), 1, 2, False)
    cars = [car1, car2, car3]

    # Get a list of all the Signals
    n_signals = [Signal(j['traffic_light'], state='rood') for i, j in lane_indcoil_signal.items() if
                 j['traffic_light'] != '']
